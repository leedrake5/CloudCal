    ...
    dataHoldDeconvolution <- reactive({
       ...
       observeEvent(input$xgbmaxdeltastep, {
        ...
        nonLinearParameters <- reactive(label="nonLinearParameters", {
            list(CalTable=calConditionsTable(cal.type=2, line.type=input$linepreferenceelement, line.structure=input$linestructure, gaus.buffer=input$gausbuffer, split.buffer=input$splitbuffer, norm.type=basichold$normtype, norm.min=basichold$normmin, norm.max=basichold$normmax, compton.type=input$comptontype, dependent.transformation=dependentTransformation()), StandardsUsed=vals$keeprows, Scale=list(Min=yMin(), Max=yMax()))
        })
